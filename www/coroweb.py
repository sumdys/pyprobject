...
def has_var_kw_arg(fn):
	params = inspect.signature(fn).parameters
	for name, param in params.items():
		if param.kind == inspect.Parameter.VAR_KWYWORD:
			return True
...
def has_request_arg(fn):
	...
	for name, param in params.items():
		if name == 'request':
			found = True
			continue
		if found and (param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Parameter.VAR_KWYWORD):
...
class RequestHandler(object):
	...
	async def __call__(self, app, fn):
		kw = None
...
def add_route(app, fn):
	...
	path = getattr(fn, '__routor__', None)
