...
def text2html(text):
	lines = map(lambda s: '<p>%s</p>' % s.replace('&','&amp').replace('<','&gt'),filter(lambda s: s.strip() != '', text.split('\n')))
	return ''.join(lines)
...
async def index(request):
	...
	blogs = [
		...
		Blog(id='2',name='Somthing New',summary=summary,created_at=time.time()-3600),
